...
<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>	
	<div class="progression-studios-property-index">
			...
			<div class="progression-studios-property-index-image">
				<a href="<?php the_permalink(); ?>">
					...
					<div class="pro-overlay-price">
						...
						<?php if(get_post_meta($post->ID, 'pyre_banner', true) == 'price-change') : ?><div class="overlay-status-pro reduced-pro"><?php echo esc_html__( 'Price Change', 'freehold-progression' ); ?></div
		...
		<div class="clearfix-pro"></div>
		<?php if(has_post_thumbnail()): ?></div><!-- close .progression-studios-property-index-has-image-content --><?php endif; ?>
			
		<div class="clearfix-pro"></div>
		
		<div class="grid2column-progression property-index-footer">
			<?php $terms = get_the_terms( $post->ID , 'property_status' );  if ( !empty( $terms ) ) : ?>
			<div class="progression-property-tax-status"><i class="fa fa-tag" aria-hidden="true"></i><span><?php esc_html_e( 'Property Status: ', 'freehold-progression' ); ?></span><?php echo get_the_term_list( $wp_query->post->ID, 'property_status', '', ', ', '' );?></div>
			<?php endif; ?>
		</div><!-- close .grid2column-progression -->
		
		<div class="grid2column-progression lastcolumn-progression property-index-footer">
			<a href="<?php the_permalink(); ?>" class="progression-property-index-button"><?php esc_html_e( 'View Listing', 'freehold-progression' ); ?></a>
		</div><!-- close .grid2column-progression -->
